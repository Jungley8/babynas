...
- **大目录分页**：每页 60 项无限滚动加载，几千首的目录（如儿歌）在旧设备也不卡
- **音视频兼容**（可选，需 ffmpeg）：浏览器不支持的编码自动处理——视频 H.265/HEVC → 转码 H.264，仅容器不对(mkv/ts) → 秒级 remux 换壳；音频 mp2/pcm 等"假后缀 mp3" → 转码 mp3。编码本就兼容 → 原样直发；未安装 ffmpeg 时自动跳过
- **三大入口**
...
担心彩屏伤眼？访问 `/lite` 进入**轻享版**：服务端渲染、**零 JS、零 fetch**、纯黑白高对比、大字大按钮，专为老旧浏览器与灰度屏设计。主页底部也有入口。

**两种用法：**

- **Kindle 墨水屏**：用「体验版浏览器」打开 `http://<NAS-IP>:8088/lite`。
  - ⚠️ 墨水屏 Kindle **本身不发声**：需型号支持蓝牙（如 Paperwhite 10 代/信号版、Oasis）并连蓝牙音箱或耳机。基础款无蓝牙则无法出声。
  - ⚠️ Kindle 浏览器极老，能否播放 HTML5 音频取决于固件；轻享版已用最兼容的原生 `<audio>` + 直连 MP3、无自动播放，给它最大成功率。
- **灰度屏手机（最省事，推荐）**：用任意手机访问 `/lite`，再开系统灰度：
  - iOS：设置 → 辅助功能 → 显示与文字大小 → **色彩滤镜** → 灰度（可设三击侧边键快捷切换）
  - 安卓：设置 → 数字健康 / 无障碍 → **灰度 / 单色模式**
  - 既护眼（无彩光蓝光），又有扬声器和正常浏览器，音频稳定可播。
